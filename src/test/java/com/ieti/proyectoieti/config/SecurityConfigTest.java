...
class SecurityConfigTest {
  ...
  void securedEndpoints_ShouldRequireAuthentication() throws Exception {
    mockMvc.perform(get("/api/wallets")).andExpect(status().is3xxRedirection());

    mockMvc.perform(get("/events")).andExpect(status().is3xxRedirection());

    mockMvc.perform(get("/api/user/profile")).andExpect(status().is3xxRedirection());
  }

  @Test
  @WithMockUser
  void securedEndpoints_WithAuthentication_ShouldBeAccessible() throws Exception {
    mockMvc.perform(get("/api/wallets")).andExpect(status().isOk());
